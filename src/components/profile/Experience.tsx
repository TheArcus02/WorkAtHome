import { AccessTimeOutlined, BusinessOutlined, KeyboardArrowDown, Work } from "@mui/icons-material"
...
const JobCard: React.FC<JobCardProps> = ({ job, index, mobileView }) => {
    return (
        <Card
            sx={!mobileView ? (index % 2 == 0 ? { maxWidth: 345 } : { alignSelf: 'flex-end', maxWidth: 345 }) : ({ maxWidth: 345 })}
            component={Paper}
            elevation={10}
        >
            <CardHeader
...
export const Experience: React.FC<ExperienceProps> = ({ jobs, isMobile }) => {
            ...
            {!isMobile ? (
                !labTimeline ? (
                    <CardsContainer sx={{ mx: 10, my: 5 }}>
                        {jobs.map((job, index) => (
                            <JobCard job={job} index={index} key={job.companyUid + index} mobileView={false} />
                        ))}
                    </CardsContainer>
                ) : (
                ...
                <Box sx={{display:'flex', flexDirection:'column', alignItems:'center', gap:2, my:4}}>
                    {jobs.map((job, index) => (
                        <>
                            <JobCard job={job} index={index} key={job.companyUid + index} mobileView={true} />
                            {jobs.length - 1 !== index  && <KeyboardArrowDown /> } 
                        </>
                    ))}
                </Box>
            )}
